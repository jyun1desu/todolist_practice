...
export default function populateList(tasksArray = [], $taskList) {
    let taskHTMLlist = tasksArray.map((task, index) => {
        ...
        <div class="main_information">
            <i id="done${index}" data-use="done" class="fas fa-check completed_checkbox ${task.done?" clicked":""}"></i>
            <input type="text" class="task_title" value="${task.taskTitle}" placeholder="Type Something Here..." readonly>
            <i id="primary${index}" data-use="primary" class="${task.primary?" clicked":""} ${task.primary?" fas":"far"}
                fa-star star_mark"></i>
    ...
    const $primaryBlock = $taskList.find('#primary_category');
    const $normalBlock = $taskList.find('#normal_category');
    const $donePrimaryBlock = $taskList.find('#done_primary_category');
    const $doneNormalBlock = $taskList.find('#done_normal_category');

    $primaryBlock.html(sortedPrimary);
    $normalBlock.html(sortedNormal);
    $donePrimaryBlock.html(sortedDonePrimary);
    $doneNormalBlock.html(sortedDoneNormal);
