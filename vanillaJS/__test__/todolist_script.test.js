...
describe('新增、刪除、修改', () => {
    ...
    test('新增任務:不是重要任務、不是完成任務，增加任務名稱first,日期2020-12-20', () => {
        ...
        const expectedResult = `<form data-index="0" ondragstart="handleDragStart(event,this)" ondragend="handleDragEnd(event,this)" ondragenter="handleDragPassby(event,this)" class="tasks  " draggable="true">
        <div class="drag_icon">
        </div>
    ...
    <button type="button" class="save_button">+ Save</button>
        ...
        //Arrange
        const container = document.body;
        const taskList = container.querySelector('.todo_list');
        const tasks = JSON.parse(localStorage.getItem('tasks'));
        const task = container.querySelector('.tasks');
        const editButton = container.querySelector("input.edit_icon");
        const date = container.querySelector(".deadline_date");
        const saveButton = container.querySelector(".save_button");
        // Act
        const populateList = require('../script/modules/populateList')
        populateList(tasks,taskList)
        fireEvent.click(editButton);
        date.value = "2020-12-30"
        saveButton.click();
        // Assert
        const hactualResult = task.innerHTML
        const hexpectedResult = `<form data-index="0" ondragstart="handleDragStart(event,this)" ondragend="handleDragEnd(event,this)" ondragenter="handleDragPassby(event,this)" class="tasks  " draggable="true">
        <div class="drag_icon">
        </div>
        <div class="main_information">
        <input id="status0" data-use="done" class="completed_checkbox" type="checkbox">
